...
```bash
git clone https://github.com/duranruizsantiago-lang/honeypot-network.git
cd honeypot-network
cp .env.example .env
docker compose -f dev.compose.yml up -d
```

Dashboard loads at `http://localhost:3000`. Connect to the SSH honeypot to see your first captured session:
...
Part of my cybersecurity portfolio. Demonstrates:

- **Network security:** Multi-protocol honeypot deployment, raw TCP protocol emulation
- **Threat intelligence:** MITRE ATT&CK mapping, IOC extraction, STIX 2.1 exports
- **Backend engineering:** Event-driven Go architecture, concurrent service simulation, real-time WebSocket streaming
- **Attacker behavior analysis:** Session recording, credential capture, tool fingerprinting
- **Operational security:** Firewall blocklist generation, GeoIP enrichment, legal compliance documentation
